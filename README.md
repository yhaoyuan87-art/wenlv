...
| 🚇 地铁空间 | 7 条线路 140 站、换乘共点、标签屏幕空间避让、线路聚焦镜头、主题一日线串线动画、Bloom 辉光（桌面端） |
| 🚄 列车模拟 | 全网 14 列三节编组列车铰接过弯、进站减速停靠，点击列车进入跟随镜头（ESC 退出） |
| 🧭 路径规划 | 站到站最短时间路径（Dijkstra + 换乘惩罚）：3D 流光动画、换乘站脉冲、行程卡片 |
| 🕳️ 站体剖面 | 选中站点镜头入地：B1 站厅 / B2 站台双层剖视、双侧站台与停站列车、出入口光柱 |
| 👻 规划线路 | 5/9 号线全息虚线幽灵层（示意走向，标注「规划中」，呼吸闪烁） |
| 🎥 开场运镜 | 首次进入地铁层：贴轨低位起步 → 翻出地面 → 全网落幅（点击跳过，每会话一次） |
